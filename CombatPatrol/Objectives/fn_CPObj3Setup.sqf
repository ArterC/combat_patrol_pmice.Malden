...
BIS_HVT = _HVTGrp createUnit [if (BIS_CP_enemySide == WEST) then {if (toLower missionName == "mp_combatpatrol_03" && toLower worldName == "tanoa") then {"rhsusf_army_ocp_officer"} else {"rhsusf_army_ocp_officer"}} else {"I_Officer_F"}, _pos, [], 0, "FORM"];
_guard1 = _HVTGrp createUnit [if (BIS_CP_enemySide == WEST) then {if (toLower missionName == "mp_combatpatrol_03" && toLower worldName == "tanoa") then {"rhsusf_army_ocp_rifleman_m4"} else {"rhsusf_army_ocp_rifleman_m4"}} else {"I_Soldier_F"}, _pos, [], 0, "FORM"];
_guard2 = _HVTGrp createUnit [if (BIS_CP_enemySide == WEST) then {if (toLower missionName == "mp_combatpatrol_03" && toLower worldName == "tanoa") then {"rhsusf_army_ocp_rifleman_m4"} else {"rhsusf_army_ocp_rifleman_m4"}} else {"I_Soldier_F"}, _pos, [], 0, "FORM"];
...
"BIS_HVTRad" setMarkerColor (if (BIS_CP_enemySide == WEST) then {"colorBLUFOR"} else {"colorIndependent"});
...
"BIS_HVTEscape" setMarkerColor (if (BIS_CP_enemySide == WEST) then {"colorBLUFOR"} else {"colorIndependent"});
