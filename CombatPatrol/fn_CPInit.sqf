...
if (count (missionNamespace getVariable ["paramsArray", []]) == 0) then {
	...
	("paramsArray is empty, using default values") call BIS_fnc_log;
};
...
BIS_CP_playerSide = EAST;
if (BIS_CP_preset_enemyFaction == 2) then {BIS_CP_preset_enemyFaction = selectRandom [0, 1]};
BIS_CP_enemySide = if (BIS_CP_preset_enemyFaction == 0) then {WEST} else {RESISTANCE};
...
BIS_fnc_CPObjSetup = compile preprocessFileLineNumbers format ["CombatPatrol\Objectives\fn_CPObj%1Setup.sqf", BIS_CP_preset_objective];
...
if (isServer) then {
	...
	BIS_CP_enemyGrp_sentry = if (BIS_CP_enemySide == WEST) then {
		if (_tanoaCamo) then {
			configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSentry"
		} else {
			//configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSentry"
			["rhsusf_army_ocp_grenadier", "rhsusf_army_ocp_rifleman"]
		};
	} else {
		configFile >> "CfgGroups" >> "Indep" >> "IND_F" >> "Infantry" >> "HAF_InfSentry"
	};
	BIS_CP_enemyGrp_fireTeam = if (BIS_CP_enemySide == WEST) then {
		if (_tanoaCamo) then {
	...
	BIS_CP_enemyGrp_rifleSquad = if (BIS_CP_enemySide == WEST) then {
		if (_tanoaCamo) then {
	...
	BIS_CP_enemyVeh_MRAP = if (BIS_CP_enemySide == WEST) then {if (_tanoaCamo) then {"O_T_LSV_02_unarmed_F"} else {"O_MRAP_02_F"}} else {"I_MRAP_03_F"};
	BIS_CP_enemyVeh_Truck = if (BIS_CP_enemySide == WEST) then {
		if (_tanoaCamo) then {
	...
	BIS_CP_enemyVeh_UAV_big = if (BIS_CP_enemySide == WEST) then {"O_UAV_02_F"} else {"I_UAV_02_F"};
	BIS_CP_enemyVeh_UAV_small = if (BIS_CP_enemySide == WEST) then {"O_UAV_01_F"} else {"I_UAV_01_F"};
	
	BIS_CP_enemyVeh_reinf1 = if (BIS_CP_enemySide == WEST) then {if (_tanoaCamo) then {"O_T_APC_Wheeled_02_rcws_ghex_F"} else {"O_APC_Wheeled_02_rcws_F"}} else {"I_APC_Wheeled_03_cannon_F"};
	BIS_CP_enemyVeh_reinf2 = if (BIS_CP_enemySide == WEST) then {if (_tanoaCamo) then {"O_T_APC_Tracked_02_cannon_ghex_F"} else {"O_APC_Tracked_02_cannon_F"}} else {"I_APC_tracked_03_cannon_F"};
	BIS_CP_enemyVeh_reinfAir = if (BIS_CP_enemySide == WEST) then {"O_Heli_Transport_04_covered_F"} else {"I_Heli_Transport_02_F"};
...
if (isServer) then {
	...
	if (_playersNo >= 8) then {
		[BIS_CP_enemyGrp_sentry, {random 600}, 12] call ARTR_fnc_CPSpawnGarrisonGrp;
		if (BIS_CP_preset_garrison == 0) then {
			[BIS_CP_enemyGrp_sentry, {random 400}, 6] call ARTR_fnc_CPSpawnGarrisonGrp;
		} else {
			[BIS_CP_enemyGrp_fireTeam, {random 400}, 6] call ARTR_fnc_CPSpawnGarrisonGrp;
		};
		if (BIS_CP_preset_garrison == 2) then {
			[BIS_CP_enemyGrp_rifleSquad, {random 200}, 3] call ARTR_fnc_CPSpawnGarrisonGrp;
		};
	} else {
		if (_playersNo >= 4) then {
			[BIS_CP_enemyGrp_sentry, {random 600}, 12] call ARTR_fnc_CPSpawnGarrisonGrp;
			if (BIS_CP_preset_garrison == 0) then {
				[BIS_CP_enemyGrp_sentry, {random 400}, 4] call ARTR_fnc_CPSpawnGarrisonGrp;
			} else {
				[BIS_CP_enemyGrp_fireTeam, {random 400}, 4] call ARTR_fnc_CPSpawnGarrisonGrp;
			};
			if (BIS_CP_preset_garrison == 2) then {
				[BIS_CP_enemyGrp_rifleSquad, {random 200}, 2] call ARTR_fnc_CPSpawnGarrisonGrp;
			};
		} else {
			[BIS_CP_enemyGrp_sentry, {random 600}, 8] call ARTR_fnc_CPSpawnGarrisonGrp;
			if (BIS_CP_preset_garrison == 0) then {
				[BIS_CP_enemyGrp_sentry, {random 400}, 4] call ARTR_fnc_CPSpawnGarrisonGrp;
			} else {
				[BIS_CP_enemyGrp_fireTeam, {random 400}, 4] call ARTR_fnc_CPSpawnGarrisonGrp;
			...
			if (BIS_CP_preset_garrison == 2) then {
				[BIS_CP_enemyGrp_rifleSquad, {random 200}, 1] call ARTR_fnc_CPSpawnGarrisonGrp;
			};
		};
	};
	...
	BIS_CP_detectedTrg setTriggerActivation [str BIS_CP_playerSide, if (BIS_CP_enemySide == WEST) then {"WEST D"} else {"GUER D"}, FALSE];
